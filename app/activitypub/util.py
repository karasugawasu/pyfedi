...
def update_post_from_activity(post: Post, request_json: dict):
    ...
    with redis_client.lock(f"lock:post:{post.id}", timeout=30, blocking_timeout=6):
        ...
        if 'name' in request_json['object']:
            ...
            if 'summary' in request_json['object'] and request_json['object']['summary']:
                autogenerated_title, link = microblog_content_to_title(request_json['object']['summary'])
            else:
                autogenerated_title, link = microblog_content_to_title(post.body_html)
            if len(autogenerated_title) < 5:
                ...
                if json_tag['type'] == 'Hashtag':
                    ...
                    if post.microblog:
                        if not isinstance(name, str) or not name:
                            continue
                        if post.microblog and name.lower() in ('#link', '#text'):
                            continue
