...
def allowlist_html(html: str, a_target='_blank', test_env=False) -> str:
    ...
    if emoji_replacements:
        ...
        clean_html = pattern.sub(
            lambda m: "<img class='emoji' referrerpolicy='no-referrer' width=30 height=30 src='" + emoji_replacements[m.group(0).lower()] + "'>",
            clean_html
        )
    ...
    if '<h1>' in html.lower():
        ...
        for p_tag in p_tags:
            ...
            for br in p_tag.find_all('br'):
                br.replace_with('\n')
            potential_title = p_tag.get_text(separator=" ", strip=False)
            for line in (ln.strip() for ln in potential_title.splitlines()):
                if line:
                    title = line
                    break
            if title:
